...
2026 Q3 审读中在生态里观察到、正在考虑吸收的字段。尚未强制；linter 目前仅按未知键给警告（DSK010w）。

- `whenToUse` / `trigger` / `do-not-trigger` — 把触发与反触发条件提升为一等字段，而不是塞在 `description` 的散文里。
- `tags` — 自由标签列表，用于发现。
- `official` — 布尔值；由宿主或平台方官方发布的技能。

欢迎对本节提 PR，附一个真实使用该字段的技能。
